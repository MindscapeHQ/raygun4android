...
object RaygunLogger {
    ...
    fun responseCode(responseCode: Int) {
        when (responseCode) {
            ...
            RaygunSettings.RESPONSE_CODE_BAD_MESSAGE -> e(
            ...
            RaygunSettings.RESPONSE_CODE_INVALID_API_KEY -> e(
                "Invalid API Key - The value specified in the header X-ApiKey did not match"
                        + " with an application in Raygun"
            )

            RaygunSettings.RESPONSE_CODE_LARGE_PAYLOAD -> e(
                "Request entity too large - The maximum size of a JSON payload is 128KB"
            )

            RaygunSettings.RESPONSE_CODE_RATE_LIMITED -> e("Too Many Requests - Plan limit exceeded for month or plan expired")
